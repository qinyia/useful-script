...
obsdir=/GPFS/cess1/omwg/pop_obs_data_20100719/phc/
cp $obsdir/PHC2_TEMP_s05_ann_avg_L46.nc $workdir

ncap2 -O -s 'depth*=100' PHC2_TEMP_s05_ann_avg_L46.nc PHC2_TEMP_s05_ann_avg_L46_cm.nc

ncwa -O -m REGION_MASK -a nlat,nlon -w TAREA -v TEMP PHC2_TEMP_s05_ann_avg_L46_cm.nc PHC2.s05.global.avg.all.mean.nc
# change dimension name from depth to z_t
ncrename -v depth,z_t -d depth,z_t PHC2.s05.global.avg.all.mean.nc

# calculate difference b/t modeling result and observation
ncdiff -O TEMP.global.avg.all.mean.nc PHC2.s05.global.avg.all.mean.nc model-obs.temp.nc
